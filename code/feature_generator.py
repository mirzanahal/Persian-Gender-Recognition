...
def generate_features(path):
    ...
    #embedding_gensim
    if (not isfile(join(data_path.DATA_PATH, path, 'embedded_texts_gensim.csv'))):
        system('python generate_embedding_gensim.py --verbose --output {}'.format(path))
    embedding_features = pd.read_csv(join(data_path.DATA_PATH, path, 'embedded_texts_gensim.csv'), index_col=0)
